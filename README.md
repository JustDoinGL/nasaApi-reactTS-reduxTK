...
1. Краткое описание каждой папки в структуре проекта:
   ...
   ![Структрура компонент](./helpers/ProjectStructureMain.png) 
   1. actions - папка, содержащая вынесенную логику приложения, которая должна переиспользоваться.
   2. assets - папка, содержащая картинки или SVG-изображения.
   3. components - папка, содержащая компоненты, которые используются на страницах.
   4. hooks - папка, содержащая кастомные хуки.
...
# Доработки
1. Сделал оптимизацию с помощью ленивых вычислений, но из-за маленького приложения это оказалось больше в минусе, чем в плюсе, но я все равно оставлю так. 😕
